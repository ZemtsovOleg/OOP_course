'''
В python существует модуль tkinter , помогающий создавать графические интерфейсы. Внутри этого модуля есть класс Label , который отвечает за отображения текста на экране.
Если изучить документацию по созданию экземпляра класса Label , то увидите множество атрибутов, которое он может принимать.
И все эти атрибуты, кроме master, не являются обязательными. Это значит, что мы можем создать экземпляры от Label  и передавать произвольное количество именованных аргументов.
...
label3 = CustomLabel(text="Password", font=(
    "Comic Sans MS", 24, "bold"), bd=20, bg='#ffaaaa')
